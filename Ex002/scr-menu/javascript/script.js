...
function init() {
  ...
  container.appendChild(createSection('Horários de funcionamento', horarios));

  const endereco = ["Local tal, na rua tal"];
  container.appendChild(createSection('Endereço', endereco));

  const tempoEntrega = ["110min - 115min"];
  container.appendChild(createSection('Tempo de entrega', tempoEntrega));

  const opcoesEntrega = [
      '<i class="fa-solid fa-motorcycle"></i> Delivery',
      '<i class="fa-solid fa-bag-shopping"></i> Retirada'
  ];
  container.appendChild(createSection('Opções de entrega', opcoesEntrega));

  const pagamentoEntrega = [
      "Dinheiro",
      "PIX (pagamento antecipado no whatsapp)"
  ];
  container.appendChild(createSection('Pagamento na entrega', pagamentoEntrega));
  ...
  container.appendChild(createSection('Cartão de crédito', cartaoCredito));
  ...
  container.appendChild(createSection('Cartão de débito', cartaoDebito));

  const leCardAlimentacao = ["LE CARD"];
  container.appendChild(createSection('LE CARD - ALIMENTAÇÃO', leCardAlimentacao));

  const leCardRefeicao = ["LE CARD"];
  container.appendChild(createSection('LE CARD - REFEIÇÃO', leCardRefeicao));

  const picpay = ["PICPAY"];
  container.appendChild(createSection('PICPAY (pagamento antecipado no whatsapp)', picpay));

  const estabelecimento = [
      "Razão Social: Pizza Bliss",
      "CNPJ: 00.000.000/0000-00"
  ];
  container.appendChild(createSection('Estabelecimento', estabelecimento));
}

// Inicializar o conteúdo ao carregar a página
