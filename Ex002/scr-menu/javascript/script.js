...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // Obter o elemento do cardápio
    const cardapio = document.getElementById('cardapio');

    // Limpar o cardápio existente
    cardapio.innerHTML = '';

    // Verificar se deve mostrar o menu de pizzas
    if (data.usarMenuPizza) {
      // Exibir as categorias de Pizza Bliss
      const pizzaCategorias = data.categorias.slice(0, 2); // As duas primeiras categorias são de pizzas
      pizzaCategorias.forEach(categoria => exibirCategoria(categoria, cardapio));
    }

    // Verificar se deve mostrar o menu de Cake Bliss
    if (data.usarMenuCake) {
      // Exibir a categoria Cake Bliss
      const cakeCategoria = data.categorias[2]; // A terceira categoria é de bolos
      exibirCategoria(cakeCategoria, cardapio);
    }

    // Sempre exibir os refrigerantes
    const refrigeranteCategoria = data.categorias[3]; // A quarta categoria é de refrigerantes
    exibirCategoria(refrigeranteCategoria, cardapio);
...
function exibirCategoria(categoria, cardapio) {
